...
private:
    ...
    void initVulkan() {
        ...
        as::create_vulkan_instance(instance, instance_create_info); //createInstance();
        if (instance_create_info.enable_validation_layers)
        {
			as::setup_debug_messenger(&instance, &debugMessenger); //setupDebugMessenger();
        }
		as::create_surface(surface, instance, window);// createSurface();
        as::pick_physical_device(&physicalDevice, &msaaSamples, &instance, &surface); //pickPhysicalDevice();
        as::create_logical_device(&device, &graphicsQueue, &presentQueue, &physicalDevice, &surface, deviceExtensions, validationLayers); //createLogicalDevice();
        as::create_swap_chain(&swapChain, &swapChainImages, &swapChainImageFormat, &swapChainExtent, &device, &physicalDevice, &surface, window); //createSwapChain();
        as::create_image_views(&swapChainImageViews, &swapChainFramebuffers, &swapChainImages, &swapChainImageFormat, &device);//createImageViews();
        as::create_render_pass(swapChainImageFormat, msaaSamples, renderPass, device, physicalDevice); //createRenderPass();
        as::create_descriptor_set_layout(device, descriptorSetLayout); //createDescriptorSetLayout();
        as::create_graphics_pipeline(graphicsPipeline, pipelineLayout, device, msaaSamples, descriptorSetLayout, renderPass); //createGraphicsPipeline();
...
public:
	...
	static char* readFile_str(const char* filename) 
    {
...
//        compile_info.file_name = new char[]("fragment_shader");
//
//		std::vector<char> shader_source = HelloTriangleApplication::readFile("shader.frag");
//		 //compile_info.source = (char*)malloc(sizeof(char) * shader_source.size() + 200);
//		 //compile_info.source = shader_source.data();
//
//        compile_info.source = shader_source.data(); /*HelloTriangleApplication::readFile_str("shader.frag");*/
//
//        std::cout << compile_info.source;
//
//		for (char i : shader_source)
//			std::cout << i;
//
//		as::shader_binaries out_shader_bin;
//		//as::compile_shader(&out_shader_bin, compile_info); 
//       // app.run();
