...
#define AS_PATH_EDITOR_PERFORMANCE_SCENE 		"../resources/scenes/editor_performance_scene.as_scene"

#define AS_PATH_EDITOR_DEFAULT_VERT_SHADER 		"../resources/shaders/editor/editor_default_vert.glsl"
#define AS_PATH_EDITOR_DEFAULT_FRAG_SHADER 		"../resources/shaders/editor/editor_default_frag.glsl"
...
#define AS_PATH_EDITOR_SKY_FRAG_SHADER 			"../resources/shaders/editor/editor_sky_frag.glsl"
...
void as_editor_set_default_scene()
{
	...
	as_camera* camera = as_camera_create(AS_VEC_PTR(as_vec3, -36.f, -4.f, 2.f), AS_VEC_PTR(as_vec3, 0.f, 0.f, 10.f));
	as_camera_set_view(camera, AS_CAMERA_MANUAL);
...
void as_editor_set_performance_scene()
{
	as_scene* editor_scene = as_scene_create(as_engine_get_render(), AS_PATH_EDITOR_PERFORMANCE_SCENE);
	as_engine_set_scene(editor_scene);

	as_camera* camera = as_camera_create(AS_VEC_PTR(as_vec3, 300., 300., 400.), AS_VEC_PTR(as_vec3, 0.f, 0.f, 0.f));
...
#pragma region SCENE
	...
	as_object* object_default = as_object_create(shape_sphere, shader_default);
	as_object_set_instance_count(object_default, 35000);
#pragma endregion SCENE
